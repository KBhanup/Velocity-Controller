...
load('Z_Input_Joy_cmd.mat');
load('Z_Output_pose.mat');
load('Z_VZ_Bdyvel.mat');
load('Z_wrld_qw.mat');
load('Z_wrld_qx.mat');
load('Z_wrld_qy.mat');
load('Z_wrld_qz.mat');
...
Kp = 1.3131;
Kd = 0.032893;
X_est = zeros([2, size(Sel_Pose_Z, 1)]);
X_est(1,1)=1;
for j= 2:1781
    e_k(j-1,1) = P_d - P_C(1,j-1);
    e_p(j,1) = e_k(j-1,1);
    U(j-1,1) = input_mesu(e_k(j-1,1), e_p(j-1,1), Kp, Kd);
    Vk_z(j-1,1) = vel_mesu_z(V_Z_BodyVel(j-1,1), U(j-1,1));
    VinW(1,j-1) = velto_w(wrld_qw(j-1,1), wrld_qx(j-1,1), wrld_qy(j-1,1), wrld_qz(j-1,1), Vk_z(j-1,1));
...
%Function for  Z
function Vk_z = vel_mesu_z(V_Z_BodyVel, U)  % This would be in Body frame y (measurment for given U input)
         Vk_z = (0.1912/(1-0.818*V_Z_BodyVel))*U;        
end
